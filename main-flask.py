...
from src.common import lxsecurity
from src.common import Httpx
from src.modules import handleApiRequest
    ...
    if config.read_config("security.allowed_host.enable"):
        if request.remote_host.split(":")[0] not in config.read_config(
            ...
            if config.read_config("security.allowed_host.blacklist.enable"):
                config.ban_ip(
                    request.remote_addr,
                    int(config.read_config("security.allowed_host.blacklist.length")),
                )
            return handleResult({"code": 6, "msg": "未找到您所请求的资源", "data": None}), 404
